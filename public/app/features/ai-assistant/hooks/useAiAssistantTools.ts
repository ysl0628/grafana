...
export const useAiAssistantTools = (): AiAssistantToolsHookResult => {
  ...
  const executeToolWithContext = useCallback(async (toolCall: ToolCall): Promise<any> => {
...
export const useDashboardTools = () => {
  const { tools, executeToolCall, validatePermissions } = useAiAssistantTools();

  // Get dashboard information
  const getDashboardInfo = useCallback(async (dashboardId?: string) => {
  ...
  const getDashboardPanels = useCallback(async (dashboardId?: string) => {
...
export const useDataQueryTools = () => {
  ...
  // Execute data query
  const queryData = useCallback(async (query: any) => {
  ...
  const getTimeSeries = useCallback(async (query: any) => {
  ...
  const getDatasourceInfo = useCallback(async (datasourceUid: string) => {
...
export const useNavigationTools = () => {
  ...
  const navigateToDashboard = useCallback(async (dashboardUid: string) => {
    const url = `/d/${dashboardUid}`;
    return await navigateToUrl(url);
  }, [navigateToUrl]);

  // Navigate to explore
  const navigateToExplore = useCallback(async (datasourceUid?: string) => {
...
export const useToolExecutionMonitoring = () => {
  ...
  const executeWithMonitoring = useCallback(async (toolCall: ToolCall) => {
    setIsExecuting(true);
